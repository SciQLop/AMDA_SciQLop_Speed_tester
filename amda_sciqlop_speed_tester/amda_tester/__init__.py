...
def get_amda_dl_url(product='thb_bl', start_time=datetime(2018, 10, 14, 0), stop_time=datetime(2018, 10, 14, 10),
                    token=None, base_url="http://amda.irap.omp.eu", debug=False):
    if token is None:
    ...
    url = f"{base_url}/php/rest/getParameter.php?parameterID={product}&startTime={start_time}&stopTime={stop_time}&timeFormat='UNIXTIME'&token={token}"
    if debug:
        print(url)
...
def get_from_amda(product='thb_bl', start_time=datetime(2018, 10, 14, 0), stop_time=datetime(2018, 10, 14, 10),
                  token=None, base_url="http://amda.irap.omp.eu", debug=False):
    data = requests.get(
